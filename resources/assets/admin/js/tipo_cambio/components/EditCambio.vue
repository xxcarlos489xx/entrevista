            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Editar tipo de cambio <i class="fas fa-dollar-sign float-right"></i></h4>
                    </div>
                    <div class="card-body">
                        <div class="form-row">
                            <div class="col-6">
...
export default {
    ...
    methods: {
        load(){
            this.form.tc_compra = this.tc.valor_compra;
            this.form.tc_venta = this.tc.valor_venta;
        },
        save(){
            this.$v.$touch();
            if (!this.$v.$anyError){
                let formData = new FormData();
                formData.append("id",this.tc.id)
                formData.append("tc_compra",this.form.tc_compra)
                formData.append("tc_venta",this.form.tc_venta)
                this.loading = true
                this.$TcApi.edit(formData)
                .then(({data})=>{
                    if (data.edit) {
                        this.loading = false
                        this.$swal.fire({
